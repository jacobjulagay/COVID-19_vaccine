...
us_state_vaccinations_url = "https://github.com/jacobjulagay/covid-19-data/blob/master/public/data/vaccinations/us_state_vaccinations.csv?raw=true"
...
us_state_vaccinations_df.to_sql('us_state_vaccinations', con=engine, if_exists = 'append', index=False)
age_groups_of_people_fully_vaccinated_df.to_sql('age_groups_of_people_fully_vaccinated',con=engine, if_exists = 'append', index=False)
race_ethnicity_of_people_fully_vaccinated_df.to_sql('race_ethnicity_of_people_fully_vaccinated',con=engine, if_exists = 'append', index=False)
sex_of_people_fully_vaccinated_df.to_sql('sex_of_people_fully_vaccinated',con=engine, if_exists = 'append', index=False)